...
indexers_plot = combined_queries_by_period %>% 
  ...
  filter(rank_30d < 25) %>% 
  #slice_max(order_by = rank_30d, n = 40) %>% 
  mutate(wallet_address = str_c(substr(wallet_address, 1, 4),"-",str_sub(wallet_address,-4,-1)),
         indexer_name = str_replace_all(indexer_name, "Braindexer", "🧠 Braindexer"),
         indexer_name = fct_reorder(indexer_name, rank_30d)) %>% 
  ggplot(aes(fill=period, x=indexer_name, y=queries_served, group = period,
...
new_allocations = braindexer_allocations %>% 
  ...
  mutate(total_signal = getGraphArbitrumStats("signalled_tokens"),
         ...
         nobrain_prop = round(((current_signal)/(total_signal))/((nobrain_stake)/(total_stake)),4),
         new_prop = new_prop,
         new_stake = (current_signal*total_stake)/(total_signal*new_prop),
         new_brain = if_else(current_stake < new_stake, round((new_stake), 0), 10),
