...
weekly_subgraphs = bind_rows(
  queryVolumeByDeploymentOverTime(unix_before_7, "mainnet", subgraphs_metadata),
  queryVolumeByDeploymentOverTime(unix_before_7, "matic", subgraphs_metadata),
  queryVolumeByDeploymentOverTime(unix_before_7, "arbitrum-one", subgraphs_metadata)) %>% 
  mutate(braindexer = if_else(
    deployment_id %in% braindexer_syncing, "Syncing", "Not Syncing"),
         time_period_rank = "3", time_period = "Past 7D")

fortnite_subgraphs = bind_rows(
...
```{r, echo=FALSE}
indexers_3d = combined_queries_by_period %>% 
  filter(period == "3D") %>%
  select(-period, -wallet_addresses) %>% 
  arrange(desc(queries_served)) %>% 
  mutate(queries_served = format(queries_served, nsmall=0, big.mark=","),
         query_fees_grt = format(round(query_fees_grt,2), nsmall=0, big.mark=",")) 

datatable(indexers_3d, 
          caption = "Top Indexers Serving Queries over Past 3 Days", 
          rownames = FALSE,  options = list(pageLength = 10))
```

```{r, echo=FALSE}
datatable(combined_queries_by_period %>% 
  filter(period == "30D") %>%
  select(-period, -wallet_addresses) %>% 
  arrange(desc(queries_served)) %>% 
  mutate(queries_served = format(queries_served, nsmall=0, big.mark=","),
...
datatable(braindexer_table %>% 
            ...
            select(braindexer_subgraph, query_fees, 
                   indexers, deployment_id, -period, -query_fees_grt), 
